...
app.get('/airport/:code/', async ( {code} =req.params, res) => {
    try {
        let data = await airportDb.getCodeInfo(code, 'airport');
...
app.get('/search/airport/', async (req, res) => {
    const {lat,lon, iso_country = null, type = null} = req.query;
    console.log('/search/airport',lat,lon, type, iso_country);
        let data = await airportDb.findClosetAirport(lat, lon, type, iso_country);
        console.log('data from api', data);   
});
